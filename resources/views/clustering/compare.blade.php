    ...
    @if($analysisA && $analysisB)
        ...
        <div class="bg-white rounded-3xl border border-slate-200/80 shadow-xs overflow-hidden">
            <div class="p-6 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    <h4 class="text-base font-bold text-slate-900">Tabel Perbandingan Klaster per Hari</h4>
                    <p class="text-xs text-slate-500">
                        Membandingkan <strong>{{ $analysisA->title }}</strong> vs <strong>{{ $analysisB->title }}</strong> pada tanggal yang sama
                    </p>
                </div>
            </div>
                ...
                <table class="w-full text-left text-xs">
                    <thead>
                        <tr class="bg-slate-50 text-slate-500 border-b border-slate-200 font-bold uppercase tracking-wider">
                            ...
                            <th class="py-3.5 px-4 text-right">X1 (A → B)</th>
                            <th class="py-3.5 px-4 text-right">X2 (A → B)</th>
                            <th class="py-3.5 px-4 text-right">X3 (A → B)</th>
                        </tr>
                    </thead>
                        ...
                        @foreach($comparison as $row)
                            <tr class="hover:bg-slate-50/80 transition">
                                <td class="py-3.5 px-4">
                                    <span class="font-bold text-slate-900 block">{{ $row['day_name'] }}</span>
                                </td>
                                ...
                                <td class="py-3.5 px-4 text-center whitespace-nowrap">
                                    @if($row['trend'] == 'naik')
                                        <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-emerald-100 text-emerald-800 text-[10px] font-bold">
                                            <i data-lucide="arrow-up-right" class="w-3 h-3"></i> Naik Kelas
                                        </span>
                                    @elseif($row['trend'] == 'turun')
                                        <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-rose-100 text-rose-800 text-[10px] font-bold">
                                            <i data-lucide="arrow-down-right" class="w-3 h-3"></i> Turun Kelas
                                        </span>
                                    @else
                                        <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-slate-100 text-slate-700 text-[10px] font-bold">
                                            <i data-lucide="minus" class="w-3 h-3"></i> Stabil
                                        </span>
                                    @endif
                                </td>

                                <td class="py-3.5 px-4 text-right font-mono text-slate-700 whitespace-nowrap">
                                    {{ number_format($row['x1_a'], 0, ',', '.') }} → {{ number_format($row['x1_b'], 0, ',', '.') }}
                                </td>
                                <td class="py-3.5 px-4 text-right font-mono text-slate-700 whitespace-nowrap">
                                    {{ number_format($row['x2_a'], 0, ',', '.') }} → {{ number_format($row['x2_b'], 0, ',', '.') }}
                                </td>
                                <td class="py-3.5 px-4 text-right font-mono text-slate-700 whitespace-nowrap">
                                    {{ number_format($row['x3_a'], 0, ',', '.') }} → {{ number_format($row['x3_b'], 0, ',', '.') }}
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
