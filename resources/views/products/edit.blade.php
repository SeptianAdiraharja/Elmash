...
@section('page_title', 'Edit Produk Olahan Lemon')
@section('page_subtitle', 'Perbarui spesifikasi produk ' . $product->name)
    ...
    <div class="bg-white rounded-3xl border border-slate-200/80 p-6 sm:p-8 shadow-xs">
        <form method="POST" action="{{ route('products.update', $product) }}" class="space-y-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <div class="relative">
                        <input type="number" 
                               ...
                               value="{{ old('raw_lemon_requirement', $product->raw_lemon_requirement) }}" 
                               required 
                ...
                <div>
                    <label for="stock" class="block text-xs font-semibold text-slate-700 uppercase tracking-wider mb-1.5">Stok Saat Ini (Unit) <span class="text-rose-500">*</span></label>
                    <input type="number" 
                           id="stock" 
                           name="stock" 
                ...
                <div class="sm:col-span-2">
                    ...
                    <textarea id="description" 
                              name="description" 
                              rows="3" 
                              class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:bg-white focus:outline-none focus:ring-2 focus:ring-emerald-500 transition">{{ old('description', $product->description) }}</textarea>
                </div>

                <!-- Is Active -->
                <div class="sm:col-span-2 flex items-center gap-3 pt-2">
                    <input type="checkbox" id="is_active" name="is_active" value="1" {{ old('is_active', $product->is_active) ? 'checked' : '' }} class="w-4 h-4 rounded text-emerald-600 focus:ring-emerald-500">
                    <label for="is_active" class="text-sm font-semibold text-slate-700 cursor-pointer">Status Produk Aktif</label>
                </div>
